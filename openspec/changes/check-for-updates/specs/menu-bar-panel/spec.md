...
The menu bar popover SHALL include a "Check for Updates" button below the "Open Main Window" button. Clicking this button SHALL trigger an immediate version check. While the check is in progress, the button SHALL be disabled. After the check completes, if a new version is found, the update notification row SHALL appear. If no new version is found, the button text SHALL briefly show "Up to date" before reverting.
